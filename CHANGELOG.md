...
### Changed
- Remove `REMEMBER_AUTH_SCHEME` from server.ts — auth scheme decision belongs in wrapper servers (remember-mcp-oauth-server), not in remember-mcp core
- Auth utility modules (`src/auth/`) remain available for wrapper servers to import
- Update design doc to reflect remote server architecture

## [3.19.1] - 2026-03-14

### Fixed
- Initialize remember-core's Firestore alongside remember-mcp's own Firestore at startup, fixing silent MemoryIndexService write failures that caused 404s on memory getById via REST service
